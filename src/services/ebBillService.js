...
export function calculateEbSplit(totalAmount, cycleMonths, houses) {
  return calculateBillSplit(totalAmount, cycleMonths, houses, (h) => !h.hasOwnEbMeter)
}

function calculateBillSplit(totalAmount, cycleMonths, houses, eligibleExtra = () => true) {
  const eligible = houses.filter((h) => h.status === 'occupied' && eligibleExtra(h))
  if (!eligible.length) return []
  const totalCents = Math.round(Number(totalAmount) * 100)
  const weights = eligible.map((h) => ({
    house: h,
    monthsOccupied: Math.max(0, Math.min(cycleMonths, Number(h.ebShareOverrideMonths ?? cycleMonths))),
  }))
...
export async function listEbBillCycles({ houseId } = {}) {
  const base = houseId
