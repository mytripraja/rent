...
export default function RentHistory() {
  ...
  return (
    <div className="bg-paper-raised rounded-2xl border border-brass/20 shadow-sm p-5">
      <h3 className="font-semibold text-ink mb-3">Rent History</h3>
      {payments.length === 0 && <p className="text-sm text-ink-soft">No submissions yet.</p>}
      <div className="space-y-2">
        {payments.map((p) => (
          <div key={p.id} className="flex items-center justify-between border-b border-brass/15 pb-2">
            <div>
              <p className="text-sm text-ink">{p.month} · ₹{p.amount}</p>
