    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <nav className="rm-mobile-nav lg:hidden fixed bottom-0 inset-x-0 bg-paper-raised/95 backdrop-blur-xl border-t border-[var(--rm-border)] flex items-stretch z-40 shadow-[0_-8px_24px_rgba(23,32,51,.08)]" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }} aria-label="Primary">
        {TABS.filter(t => MOBILE_PRIMARY.includes(t.id)).map(t => {
          const active = tab === t.id
          return <button key={t.id} onClick={() => go(t.id)} className={`flex-1 min-w-0 flex flex-col items-center justify-center gap-1 py-2 px-1 text-[10px] leading-none ${active ? 'text-brand font-bold' : 'text-ink-soft'}`} aria-current={active ? 'page' : undefined}><span className={`w-9 h-7 rounded-lg flex items-center justify-center ${active ? 'bg-brand/10' : ''}`}><t.icon size={18} /></span><span className="truncate max-w-full">{t.mobileLabel || t.label}</span></button>
        })}
      </nav>
