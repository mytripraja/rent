...
export default function ApartmentOperations() {
  const { user } = useAuth()
  ...
  async function createApartment() {
    ...
    try {
      ...
      setNewProperty({name:'',address:''}); await loadProperties(); setActivePropertyId(id); setActiveId(id); window.location.reload()
    } catch (e) { setError(e.message) } finally { setSaving(false) }
  }

  async function saveMotors() {
    setSaving(true)
    try { await saveApartmentOperations(activeId, { motors: currentMotors, tanks: currentTanks, settings }); await loadData(activeId); showToast({message:'Motor settings saved',type:'success'}) }
    catch(e){setError(e.message)} finally{setSaving(false)}
  }
