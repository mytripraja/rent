...
export default function IndiaCalendar({ compact=false }) {
  ...
  return <div className={`space-y-5 ${compact ? '' : 'rm-card p-4 sm:p-6'}`}>
    ...
    <section className="rounded-2xl border border-[var(--rm-border)] bg-paper p-3 sm:p-4">
      ...
      <div className="rm-calendar-grid grid grid-cols-7 gap-1 text-center text-[10px] sm:text-xs font-bold text-ink-soft">{WEEK.map(d=><div key={d} className="py-1">{d}</div>)}</div>
      <div className="rm-calendar-grid grid grid-cols-7 gap-1">
      ...
      <div className="rm-calendar-mobile-list mt-2 space-y-1.5">
        {monthDays.map(day => { const d=isoDay(day), tf=tamilDateFor(d), fest=festivals.filter(f=>f.date===d), mine=notes.some(n=>n.date===d), event=events.some(e=>e.date===d); return <button key={`mobile-${d}`} onClick={()=>setSelected(d)} className={`w-full text-left rounded-xl border p-3 flex items-center gap-3 ${d===selected?'border-brand bg-brand/5':'border-[var(--rm-border)] bg-paper-raised'}`}><span className="w-11 shrink-0 text-center"><span className="block text-lg font-extrabold text-ink">{day.getDate()}</span><span className="text-[10px] text-ink-soft">{day.toLocaleDateString('en-IN',{weekday:'short'})}</span></span><span className="min-w-0 flex-1"><span className="block text-xs font-semibold text-brand truncate">தமிழ்: {tf.month} {tf.day}</span><span className="block text-xs text-ink-soft truncate">{fest[0]?.title || (mine ? 'Personal event' : event ? 'Shared event' : 'No event')}</span></span><span className="text-[10px] font-bold text-ink-soft">{fest.length ? `${fest.length} fest.` : ''}</span></button> })}
      </div>
    </section>

    <div className="grid lg:grid-cols-[1.15fr_.85fr] gap-4 min-w-0">
      <section className="rounded-2xl border border-[var(--rm-border)] bg-paper p-4 min-w-0 overflow-hidden">
