...
export default function ServiceContactsList({ contacts, renderAction }) {
  ...
  if (grouped.length === 0) {
    return <p className="text-sm text-ink-soft">No service contacts added yet.</p>
  }
      ...
      {grouped.map((g) => (
        <div key={g.cat}>
          ...
          <div className="space-y-2">
            {g.items.map((c) => (
              <div key={c.id} className="flex items-center justify-between bg-paper rounded-lg px-3 py-2.5 text-sm">
                <div>
